type Props = {
  status: string;
  variant?: "default" | "live" | "open" | "upcoming" | "hosted" | "auction";
};
...
const styles: Record<NonNullable<Props["variant"]>, string> = {
  ...
  auction: "bg-fuchsia-500/10 text-fuchsia-300 ring-fuchsia-500/25",
};
    ...
    variant !== "default"
      ...
      : status === "Live" || status === "IN_PROGRESS"
        ...
        : status === "Open" || status === "REGISTRATION_OPEN"
          ? "open"
          : status === "Auction" || status === "AUCTION_LIVE"
            ? "auction"
            : status === "Upcoming" ||
                status === "UPCOMING" ||
                status === "Post-Auction" ||
  ...
  const label =
    status === "REGISTRATION_OPEN" || status === "Open"
      ...
      : status === "IN_PROGRESS" || status === "Live"
        ? "Live"
        : status === "AUCTION_LIVE" || status === "Auction"
          ? "Auction Live"
          : status === "AUCTION_COMPLETED" || status === "Post-Auction"
            ? "Auction Done"
